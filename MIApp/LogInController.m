...
@synthesize logInButton, usernameTextField, passwordTextField,plantPicker;
...
-(void) viewDidLoad
{
    ...
    [self.view insertSubview:background belowSubview:logInButton];
    plantData = @[@"Aberfeildie",@"Alouette",@"Ash_River",@"Bridge_River_1",@"Bridge_River_2",@"Cheakamus",@"Clowhom",@"Duncan_Dam",@"Elko",@"Falls_River",@"GM_Shrum",@"Hugh_L_Keenleyside",@"John_Hart",@"Jordan_River",@"Kootenay_Canal",@"La_Joie",@"Ladore",@"Lake_Buntzen",@"Mica",@"Peace_Canyon",@"Puntledge",@"Revelstoke",@"Ruskin",@"Seton",@"Seven_Mile",@"Shuswap",@"Spillimacheen",@"Stave_Falls",@"Strathcona",@"Wahleach",@"Walter_Hardman"];
    
    [[QuestionList sharedInstance] setPlantName: @"Aberfeildie"];
}
...
// For UIPicker
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)plantPicker{
    return 1;
}

- (NSInteger)pickerView:(UIPickerView *)plantPicker numberOfRowsInComponent:(NSInteger)component{
    return plantData.count;
}

- (NSString *)pickerView:(UIPickerView *)plantPicker titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    return plantData[row];
}


- (void)pickerView:(UIPickerView *)plantPicker didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    //NSString *plantName = @"Ash River";
    NSString *plantName = plantData[row];
    [[QuestionList sharedInstance] setPlantName:plantName];
}
